...
- `TeamDelete`: Agent Teams 削除（次のISSUEに進む前にクリーンアップ）
...
### 引数が指定されていない場合
`gh issue list --state open --label bug --json number,title` で未修正ISSUEを取得し、全てを対象とする。
...
例: Issue#25 で AC-16 に対応 → `grep -c "AC-16-" docs/design.md`（Issue番号とAC番号の対応はIssue本文に記載されている）
...
1. `TeamCreate` でチーム `fix{n}` を作成する（既存チームがある場合はスキップ）
2. **implementer**（`name: "implementer"`, `subagent_type: "implementer"`）と **test-writer**（`name: "test-writer"`, `subagent_type: "test-writer"`）を `team_name: "fix{n}"` で並列起動する
...
**code-reviewer**（`name: "code-reviewer"`, `subagent_type: "code-reviewer"`）、**ut-validator**（`name: "ut-validator"`, `subagent_type: "ut-validator"`）、**it-validator**（`name: "it-validator"`, `subagent_type: "it-validator"`）を `team_name: "fix{n}"` で並列起動する。
...
**`TeamDelete` でチーム `fix{n}` を削除してから、即座にフェーズ1に戻る — ここで停止してはいけない。**
